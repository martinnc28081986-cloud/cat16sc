...
def scrape_all():
    ...
    team_results  = {cat: {} for cat in CATEGORIES}  # todos los equipos, mismo formato que sc_results
        ...
        if elem.name == "table" and current_fecha is not None:
            ...
            for row in rows[1:]:
                ...
                for cat, col in cat_cols.items():
                    ...
                    # team_results: TODOS los equipos con formato unificado {fecha,rival,cond,gf,gc}
...
def main():
    ...
    for attempt in range(1, 4):
        ...
        try:
            ...
            sc_results, rival_results, team_results, all_matches = scrape_all()
